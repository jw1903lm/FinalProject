...
<html>
    <%  
        ...
        if (adminName == null || "".equals(adminName)) {
            ...
            sessionObj.setMaxInactiveInterval(30);
        }%>
    <head>
        <meta charset="utf-8">
                        ...
                        <div class="col-xs-12">
                            <div class="box">
                                <div class="box-header">
                                    <h3 class="box-title">Shop Information Data Table</h3>
                                </div>
                                <!-- /.box-header -->
                                <div class="box-body">
                                    <table id="example2" class="table table-bordered table-hover">
                                        ...
                                        <tbody>
                                            <c:forEach items="${listShopInformation}" var="shi">
                                                <tr>
                                                    <td>${shi.shopInformationId}</td>
                                                    <td>${shi.logoTop}</td>
                                                    <td>${shi.logoBottom}</td>
                                                    <td>${shi.slogan}</td>
                                                    <td>${shi.shopImage}</td>
                                                    <td>${shi.shopDescription}</td>
                                                    <td>${shi.founderImage}</td>
                                                    <td>${shi.founderQuote}</td>
                                                    <td>${shi.contactEmail}</td>
                                                    <td>${shi.contactPhone}</td>
                                                    <td>${shi.contactAddress}</td>
                                                    <td><fmt:formatDate value="${shi.created}" pattern="yyyy-MM-dd HH:mm:ss" /></td>
                                                    <td>${shi.isDisabled}</td>
                                                    <td><a class="glyphicon glyphicon-pencil" title="Update" href="${contextPath}/ShowyClothes/adminShopInformationsController/initUpdate.htm?shipId=${shi.shipId}"/>
                                                        &nbsp;
                                                        <a class="glyphicon glyphicon-remove" title="Delete" href="${contextPath}/ShowyClothes/adminShopInformationsController/delete.htm?shipId=${shi.shipId}"/></td>
                                                </tr>
                                            </c:forEach>
                                        </tbody>
                                    </table>
                                    <button><a style="text-decorations:none; color:inherit;" href="${contextPath}/ShowyClothes/adminShopInformationsController/initInsert.htm">Add Ship</a></button>
                                </div>
                            </div>
                        </div>
                    </div>
