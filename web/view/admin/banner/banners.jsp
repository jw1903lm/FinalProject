                            ...
                            <div class="box">
                                <form id="doSearch" method="post"  action="doSearch.htm">
                                    <div class="box-header">
                                        <span style="margin-top: 10px" class="glyphicon glyphicon-search" onclick="document.getElementById('doSearch').submit();">&nbsp;</span><input style="border:none" placeholder="Search Banner Name" name="search"/>
                                    </div>
                                </form>
                                <!-- /.box-header -->
                                <div class="box-body">
                                    <table id="example2" class="table table-bordered table-hover">
                                        ...
                                        <tbody>
                                            <c:forEach items="${listBanner}" var="ban">
                                                <tr>
                                                    <td>${ban.bannerId}</td>
                                                    <td>${ban.bannerName}</td>
                                                    <td>${ban.bannerDescription}</td>
                                                    <td><img style="width: 200px;height: auto" src="<%=request.getContextPath()%>/view/img/${ban.bannerImage}" alt=""></td>
                                                    <td>${ban.size}</td>
                                                    <td>${ban.catagories.catagoryName}</td>
                                                    <td><fmt:formatDate value="${ban.created}" pattern="yyyy-MM-dd HH:mm:ss" /></td>
                                                    <td>${ban.isDisabled}</td>
                                                    <td style="text-align: center"><a class="glyphicon glyphicon-pencil" title="Update" href="${contextPath}/ShowyClothes/adminBannersController/initUpdate.htm?bannerId=${ban.bannerId}"/>
                                                        &nbsp;
                                                        <a class="glyphicon glyphicon-plus" title="Add new" href="${contextPath}/ShowyClothes/adminBannersController/initInsert.htm"/></td>
                                                </tr>
                                            </c:forEach>
                                        </tbody>
                                    </table>
