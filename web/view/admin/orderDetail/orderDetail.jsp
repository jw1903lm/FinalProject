                            ...
                            <div class="box">
                                ...
                                <div class="box-body">
                                    <table id="example2" class="table table-bordered table-hover">
                                        ...
                                        <tbody>
                                            <c:forEach items="${listOrderDetail}" var="ord">
                                                <tr>
                                                    <td>${ord.orders.orderId}</td>
                                                    <td>${ord.products.productName}</td>
                                                    <td>${ord.sizes.sizeName}</td>
                                                    <td>${ord.quantity}</td>
                                                    <td>${ord.totalPrice}</td>
                                                    <td><fmt:formatDate value="${ord.created}" pattern="yyyy-MM-dd HH:mm:ss" /></td>
                                                    <td>${ord.isDisabled}</td>
                                                    <td style="text-align: center"><a class="glyphicon glyphicon-pencil" title="Update" href="${contextPath}/ShowyClothes/adminOrderDetailsController/initUpdate.htm?orderId=${ord.orders.orderId}&productId=${ord.products.productId}&sizeId=${ord.sizes.sizeId}"/>
                                                </tr>
                                            </c:forEach>
                                        </tbody>
                                    </table>
