                            ...
                            <div class="box">
                                <form id="doSearch" method="post"  action="doSearch.htm">
                                    <div class="box-header">
                                        <span style="margin-top: 10px" class="glyphicon glyphicon-search" onclick="document.getElementById('doSearch').submit();">&nbsp;</span><input style="border:none" placeholder="Search User Name" name="search"/>
                                    </div>
                                </form>
                                <!-- /.box-header -->
                                <div class="box-body">
                                        ...
                                        <tbody>
                                            <c:forEach items="${listUser}" var="user">
                                                <tr>
                                                    <td>${user.userId}</td>
                                                    <td>${user.userName}</td>
                                                    <td class="wrapword">${user.userPassword}</td>
                                                    <td>${user.phone}</td>
                                                    <td>${user.cities.cityName}</td>
                                                    <td><fmt:formatDate value="${user.created}" pattern="yyyy-MM-dd HH:mm:ss" /></td>
                                                    <td>${user.isDisabled}</td>
                                                    <td style="text-align: center"><a class="glyphicon glyphicon-pencil" title="Update" href="${contextPath}/ShowyClothes/adminUsersController/initUpdate.htm?userId=${user.userId}"/>
                                                        &nbsp;
                                                        <a class="glyphicon glyphicon-plus" title="Add new" href="${contextPath}/ShowyClothes/adminUsersController/initInsert.htm"/>
                                                        &nbsp;
                                                        <a class="glyphicon glyphicon-zoom-in " title="Detail" href="${contextPath}/ShowyClothes/adminUsersController/detail.htm?userId=${user.userId}"/></td>
                                                </tr>
                                            </c:forEach>
                                        </tbody>
                                    </table>
